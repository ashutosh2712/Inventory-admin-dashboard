...
router.get("/api/inventory/recent", (request, response) => {
  const inventoryRecentData = [];
  const csvFilePath = path.resolve("src/sample-data-v2.csv");

  fs.createReadStream(csvFilePath)
    .pipe(csvParser())
    .on("data", (data) => {
      const dataStr = data.timestamp.split(" ")[0];
      const parsedDate = parse(dataStr, "dd-MM-yyyy", new Date());
      inventoryRecentData.push({
    ...
    .on("end", () => {
      const sortedData = inventoryRecentData.sort(
      ...
      response.json(formattedData.slice(0, 1));
    })
    .on("error", (error) => {
      console.log("Error reading the csv file:", error);
